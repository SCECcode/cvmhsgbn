/**
 * @file cvmhsgbn.c
 * @brief Main file for CVMHSGBN library.
 * @author - SCEC 
 * @version 1.0
 *
 * @section DESCRIPTION
...
const char *cvmhsgbn_version_string = "CVMHSGBN";

int cvmhsgbn_is_initialized = 0;

/** Location of the binary data files. */
char cvmhsgbn_data_directory[2000];

/** Configuration parameters. */
cvmhsgbn_configuration_t *cvmhsgbn_configuration;

/** Holds pointers to the velocity model data OR indicates it can be read from file. */
cvmhsgbn_model_t *cvmhsgbn_velocity_model;

/** The height of this model's region, in meters. */
double cvmhsgbn_total_height_m = 0;

/** The width of this model's region, in meters. */
double cvmhsgbn_total_width_m = 0;
/*************************************/

int cvmhsgbn_ucvm_debug=0;
int cvmhsgbn_force_depth = 0;
int cvmhsgbn_zmode = VX_ZMODE_DEPTH;

/**
 * Initializes the CVMHSGBN plugin model within the UCVM framework. In order to initialize
...
int cvmhsgbn_init(const char *dir, const char *label) {
	...
	cvmhsgbn_configuration = calloc(1, sizeof(cvmhsgbn_configuration_t));
	cvmhsgbn_velocity_model = calloc(1, sizeof(cvmhsgbn_model_t));

	// Configuration file location when built with UCVM
	sprintf(configbuf, "%s/model/%s/data/config", dir, label);

	// Read the configuration file.
	if (cvmhsgbn_read_configuration(configbuf, cvmhsgbn_configuration) != UCVM_CODE_SUCCESS) {

           // Try another, when is running in standalone mode..
	   sprintf(configbuf, "%s/data/config", dir);
	   if (cvmhsgbn_read_configuration(configbuf, cvmhsgbn_configuration) != UCVM_CODE_SUCCESS) {
               cvmhsgbn_print_error("No configuration file was found to read from.");
               return UCVM_CODE_ERROR;
               } else {
	       // Set up the data directory.
	       sprintf(cvmhsgbn_data_directory, "%s/data/%s", dir, cvmhsgbn_configuration->model_dir);
           }
           } else {
	   // Set up the data directory.
	   sprintf(cvmhsgbn_data_directory, "%s/model/%s/data/%s", dir, label, cvmhsgbn_configuration->model_dir);
        }

        /* Init vx */
        if (vx_setup(cvmhsgbn_data_directory) != 0) {
          return UCVM_CODE_ERROR;
        }

	// Let everyone know that we are initialized and ready for business.
	cvmhsgbn_is_initialized = 1;
...
/* Setparam CVMHSGBN */
int cvmhsgbn_setparam(int id, int param, ...)
{
  va_list ap;
  int zmode;
  ...
  switch (param) {
    case UCVM_MODEL_PARAM_FORCE_DEPTH_ABOVE_SURF:
      cvmhsgbn_force_depth = va_arg(ap, int);
      break;
    case UCVM_PARAM_QUERY_MODE:
      ...
      switch (zmode) {
        case UCVM_COORD_GEO_DEPTH:
          ...
          if(cvmhsgbn_ucvm_debug) fprintf(stderr,"calling cvmhsgbn_setparam >>  depth\n");
          break;
...
/**
 * Queries CVMHSGBN at the given points and returns the data that it finds.
 *
 ...
 */
int cvmhsgbn_query(cvmhsgbn_point_t *points, cvmhsgbn_properties_t *data, int numpoints) {
     ...
     */
      ...
      if ((cvmhsgbn_force_depth) && (cvmhsgbn_zmode == VX_ZMODE_ELEV) &&
        ...
        if (vx_surf - VX_NO_DATA < 0.01) {
          ...
          if(cvmhsgbn_ucvm_debug) {
            fprintf(stderr,"POTENTIAL problem if cvmhsgbn surface differ from UCVM surface !!!\n");
          }
      ...
      // 1 is bad, 0 is good and anything not in HR region/ie cvmhsgbn 
...
{
  int verlen;
  verlen = strlen(cvmhsgbn_version_string);
  if (verlen > len - 1) {
    verlen = len - 1;
  }
  memset(ver, 0, len);
  strncpy(ver, cvmhsgbn_version_string, verlen);
  return UCVM_CODE_SUCCESS;
}

/**
 * Reads the configuration file describing the various properties of CVMHSGBN and populates
...
int cvmhsgbn_read_configuration(char *file, cvmhsgbn_configuration_t *config) {
	...
	if (config->utm_zone == 0 || config->model_dir[0] == '\0' ) {
		cvmhsgbn_print_error("One configuration parameter not specified. Please check your configuration file.");
		return UCVM_CODE_ERROR;
	}
...
void cvmhsgbn_print_error(char *err) {
	fprintf(stderr, "An error has occurred while executing CVMHSGBN. The error was:\n\n");
	fprintf(stderr, "%s", err);
	fprintf(stderr, "\n\nPlease contact software@scec.org and describe both the error and a bit\n");
	fprintf(stderr, "about the computer you are running CVMHSGBN on (Linux, Mac, etc.).\n");
}
...
/**
 * Init function loaded and called by the UCVM library. Calls cvmhsgbn_init.
 *
 * @param dir The directory in which UCVM is installed.
 * @return Success or failure.
 */
int model_init(const char *dir, const char *label) {
	return cvmhsgbn_init(dir, label);
}

/**
 * Query function loaded and called by the UCVM library. Calls cvmhsgbn_query.
 *
 ...
 */
int model_query(cvmhsgbn_point_t *points, cvmhsgbn_properties_t *data, int numpoints) {
	return cvmhsgbn_query(points, data, numpoints);
}

/**
 * Setparam function loaded and called by the UCVM library. Calls cvmhsgbn_setparam.
 *
 * @param id  don'care
 * @param param 
...
int model_setparam(int id, int param, int val) {
	return cvmhsgbn_setparam(id, param, val);
}

/**
 * Finalize function loaded and called by the UCVM library. Calls cvmhsgbn_finalize.
 *
 * @return Success
 */
int model_finalize() {
	return cvmhsgbn_finalize();
}

/**
 * Version function loaded and called by the UCVM library. Calls cvmhsgbn_version.
 *
 * @param ver Version string to return.
 * @param len Maximum length of buffer.
 * @return Zero
...
int model_version(char *ver, int len) {
	return cvmhsgbn_version(ver, len);
}

int (*get_model_init())(const char *, const char *) {
        return &cvmhsgbn_init;
}
int (*get_model_query())(cvmhsgbn_point_t *, cvmhsgbn_properties_t *, int) {
         return &cvmhsgbn_query;
}
int (*get_model_finalize())() {
         return &cvmhsgbn_finalize;
}
int (*get_model_version())(char *, int) {
         return &cvmhsgbn_version;
}
int (*get_model_setparam())(int, int, ...) {
         return &cvmhsgbn_setparam;
}
